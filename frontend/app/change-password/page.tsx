...
export default function ChangePasswordPage() {
  ...
  const onSubmit = async () => {
    ...
    if (newPassword !== newPassword2) return setError("新しいパスワードが一致しません。");

    setLoading(true);
    try {
      // 1) まず古いPWでサインインしてセッションを作る
      const res = await signIn({ username: email, password: oldPassword });

      // NEW_PASSWORD_REQUIRED（初回強制変更）などが来たら、この画面の想定外なので案内
      const step = res?.nextStep?.signInStep;
      if (typeof step === "string" && step !== "DONE") {
        // 初回強制変更はログイン画面側のフローで処理するのが安全
        throw new Error(`追加のサインイン手順が必要です: ${step}`);
      }

      // 2) セッションができた状態で updatePassword（旧→新）
      await updatePassword({ oldPassword, newPassword });

      setMessage("パスワードを変更しました。ログイン画面に戻ります。");

      // 変更後はサインアウトしておく（旧PWで作ったセッションを残さない）
      try {
        await signOut();
      } catch {
        // 無視
